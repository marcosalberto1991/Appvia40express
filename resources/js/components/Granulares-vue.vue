  ...
  <div class='card-body'><h1 class=''>Lista de Granulares </h1>
    ...
    <div class='col-12'>
        <!--
        <b-button v-if="$can('Granulares Anadir')"  @click='anadir_registro()' type='button' class='btn btn-wangir btn-lg' data-toggle='button' size='sm' aria-pressed='false' variant='success' style='margin-bottom: 5px; margin: 5px;'>Añadir registro
        </b-button>
        -->
      <router-link v-if="$can('Granulares Anadir')" :to="{ name: 'granularesformadd',params:{id:0} }">
      ...
      <template v-slot:cell(Acciones)='data'>
          ...
          <b-button-group>
          <!--
          <b-button v-if="$can('Granulares Editar')" size='sm' variant='warning'  type='button' class='btn-sm btn btn-wangir mr-1' data-toggle='button' aria-pressed='false' style='margin-bottom: 5px; margin: 5px;'>Editar
            </b-button>
            -->
            <router-link :to="{ name: 'granularesform', params: { id: data.item.id }}">
                <a v-bind:href="'/Granulares/'+data.item.id+'/edit'" class='btn-sm btn btn-success mr-1' size='sm' style='margin-bottom: 5px; margin: 5px;'>
            ...
            <b-button v-if="$can('Granulares Eliminar')"  v-b-modal.moda-eliminar @click='eliminar_registro(data.item.id)' type='button' class='btn-sm btn btn-danger mr-1' size='sm' data-toggle='button' aria-pressed='false' style='margin-bottom: 5px; margin: 5px;'>
...
export default {
  ...
  data() {
    ...
    return {
      ...
      //{ key: 'actividad_pavimento', sortable:true,class:'one-lineas'},
  ...
  methods: {
    ...
    consulta(page = 1){
      if (localStorage.getItem("Granulares")) {
        try { this.items = JSON.parse(localStorage.getItem("Granulares"));
        } catch (e) { localStorage.removeItem("Granulares");}
      }

      this.page=page;
      //axios.get("Granulares/consulta?page=" +page+"&consulta_data="+this.input_consulta_data)
      axios.get(`${this.$url}/Api/Granulares?page=${page}&consulta_data=${this.input_consulta_data}`)
      .then(response => {
        this.consulta_datos = response.data;
        const parsed = JSON.stringify(response.data);
        localStorage.setItem("Granulares", parsed);
      });
    },

    eliminar_registro(data_id){
    this.input_Granulares_id=data_id;
    },
    data_foraneo(){

    },
    eliminar_registro_delete(){
      var data_id=this.input_Granulares_id;
      axios.delete(`${this.$url}/Api/Granulares/${this.input_Granulares_id}`).then(response => {
